...
//
        ...
        for (Map.Entry<String, Boolean> entry : map.entrySet()) {
            if(entry.getValue() == false)
            {
                ...
                if(entry.getKey().equals(GlobalConfig.centerIp))
                tcpClientService.tcpClientRun(entry.getKey(),GlobalConfig.centerPort);
                else
                    tcpClientService.tcpClientRun(entry.getKey(),GlobalConfig.pingPort);
            }
        }
    }
    private void init()
    {
        ...
        warnHostMap.put(GlobalConfig.pingIp,false);
    }
}
